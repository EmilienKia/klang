...
| [`Set<T>`, `MutableSet<T>`, `OrderedSet<T>`, `MutableOrderedSet<T>`, `ListSet<T>`, `TreeSet<T>`, `HashSet<T>`](collections-set.md) | Generic set framework: interfaces + three concrete implementations (list-backed, AVL-tree-backed ordered set, hash-table-backed set). | `src/set.k` |
