...
- Add templates advanced features (Phase 2+ — partial specialization, variadic templates, template template parameters, etc.)
  ...
  - Known generic call-site limitations (found by Phase 12, tracked for future fix):
    ...
    - [ ] Origin-aware homing of imported template definitions: re-injected imported templates are still homed under the consumer module's root namespace (not under their origin namespace), so two *imported* templates with the same short name from different modules (`a::Box`, `b::Box`) would clash at the model/symbol level (re-injection dedups by short name in the target namespace). The instantiation `struct_type` registry is already collision-safe (keyed by an origin-qualified name via `unit::make_instantiation_registry_key`; see test `[template][instantiation][ns-collision]`), but full correctness for homonymous *imported* templates requires re-homing the re-parsed template under its origin namespace (with an origin-absolute name) in `kdi_importer::materialise_template_def`, aligning with the `is_type_available` expectation (`gen_unit.cpp`) that imported `k` symbols live under `root::k`.
