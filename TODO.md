...
- Add templates advanced features (Phase 2+ — partial specialization, variadic templates, template template parameters, etc.)
  ...
  - Known generic call-site limitations (found by Phase 12, tracked for future fix):
    ...
    - [ ] Imported generic-template instantiation identity: a consumer module ends up with two distinct `struct_type` objects for the same instantiation (e.g. `k::Optional__byte`) — one from the KDI importer (used as an imported method's return type) and one synthesised locally for construction. Pointer-based struct-identity checks then reject binding an imported rvalue to the local instantiation's copy constructor, so `o : Optional<byte> = bais.read();` fails (errors 0x000D9 / 0x000FF). Works when the rvalue comes from a local function/method. Repro: skipped test "ByteArrayInputStream read() rvalue copy-init into Optional<byte>" in `libk/libk/tests/test-io-byte-array-streams.cpp`. Fix: unify imported and locally-synthesised instantiations of the same generic template into a single `struct_type` (or compare struct identity by canonical instantiation name). Workaround: consume the rvalue inline via chained calls (`read().getOr(...)`, `read().hasValue()`).
