...
| `gen_adapt_type.cpp` | `adapt_type()` and `adapt_reference_load_value()` — implicit type adaptation; `adapt_callable_type()` is the single funnel every callable binding goes through |
| `gen_callable.cpp` | Callable binding resolution and codegen: `try_bind_member_callable`, `try_bind_functor_callable`, `try_bind_functional_interface_callable`, `select_callable_target`, `check_callable_conversion`, `emit_callable_invocation`, `emit_callable_nonnull_check` |
| `gen_callable_compat.cpp` | Callable compatibility rules (phase B.7): `callable_signature_compatible()` — arity, return covariance, parameter contravariance, `throws` subset, nominal-first identity, offset-0 derivation (`callable_base_at_zero_offset()`), no primitive widening/narrowing |
| `gen_callable_helpers.hpp` | `peel_to_callable`, `build_callable_value`, `extract_fn`/`extract_ctx`, `select_overload_for_prototype`, plus the `callable_signature_view` / `callable_compat` API |
...
| Work on parameterised aliases (`template<typename T> alias …`) | `parse/parser_declarations.cpp` (`parse_alias_decl`, template source capture), `model/model_builder.cpp` (`visit_alias_decl` — `tpl_info`, template-param scope), `model/model_alias.hpp` (`_tpl_info`, `_tpl_alias_types`), `gen/resolvers_scope_lookup.cpp` (`resolve_alias_template` — the shared substitution routine), `gen/resolvers_type_ref.cpp` + `gen/resolvers_aggregate.cpp` (hooks at the top of `try_instantiate_template_type`), `model/tools/kdi_exporter.cpp` / `kdi_importer.cpp` (`materialise_template_alias` — source-text round trip), tests `klang/tests/test-gen-alias-template.cpp`, spec `doc/spec/language/basic/aliases.md` §9 |
