...
All throwable types must belong to the `::k::Exception` class hierarchy defined
...
1. [Exception type constraint](#1-exception-type-constraint)
...
## 1. Exception type constraint

**Only classes derived from `::k::Exception` (or `Exception` itself) may be thrown.**

- Throwing a struct or class that does not derive from `::k::Exception` →
...
User-defined exception types must inherit from `Exception` or one of its
subclasses:
...
class NetworkError : public RuntimeException {
    public:
    NetworkError() : RuntimeException(50) { }
}
```
...
- All types in the `throws` clause must derive from `::k::Exception` (error
  `0x01C4` otherwise).
...
Any `throw` statement in the function body must throw a type that is either:
...
Any call to a function that itself has a `throws` clause must have all its
declared exception types either:
...
fetchData() : int throws NetworkError {
    err : NetworkError;
    throw err;  // OK — NetworkError is declared
}
...
- [K Standard Library — Exception Types](../../stdlib/exceptions.md) — `Exception`, `RuntimeException`, and derived types
- [Grammar — ThrowStatement, TryCatchStatement](../grammar.ebnf)
