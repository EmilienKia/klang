...
std::shared_ptr<aggregate> type_reference_resolver::find_template_aggregate(
    ...
    const element& context_elem)
{
    ...
    if (root_ns) {
        if (auto st = resolve_struct_from(*root_ns, base_name)) {
            if (st->is_template()) return st;
        }
        // For root-prefixed names (e.g. ::k::Optional), apply the same
        // module-name-stripping logic as resolve_type_from_root.
        if (base_name.has_root_prefix()) {
    ...
    if (base_name.size() > 1) {
        for (const auto& imp : _unit.get_imports()) {
            ...
            if (rest.empty()) continue;
            auto r_ns = _unit.get_root_namespace();
            if (r_ns) {
                if (auto st = resolve_struct_from(*r_ns, rest)) {
                    if (st->is_template()) return st;
                }
            }
            if (auto imp_st = _unit.get_or_create_imported_aggregate(rest, _context)) {
                auto st = std::dynamic_pointer_cast<aggregate>(imp_st);
                if (st && st->is_template()) return st;
            }
            if (rest.size() > 1 && r_ns) {
                if (auto st = resolve_struct_from(*r_ns, k::name(false, rest.back()))) {
                    if (st->is_template()) return st;
                }
            }
        }
...
std::shared_ptr<type> type_reference_resolver::instantiate_template_aggregate(
    ...
    const element& context_elem)
{
    ...
    if (tpl_agg->is_generic() && !is_imported_signature_only_generic) {
        ...
        if (concrete_agg && tpl_agg->is_generic() && ti) {
            const auto key = build_instantiation_key(model_args);
            ti->instantiations[key] = concrete_agg;
            record_generic_usage(*ti, model_args);
            if (auto site_var = dynamic_cast<const variable_definition*>(&context_elem)) {
...
type_reference_resolver::ctad_resolution_result type_reference_resolver::resolve_ctad(
    const std::shared_ptr<aggregate>& tpl_agg,
    const std::vector<std::shared_ptr<expression>>& args,
    const element& context_elem,
    const lex::opt_any_lexeme& loc_lexeme)
{
    ctad_resolution_result result;
    if (!tpl_agg || !tpl_agg->is_template()) {
        result.success = false;
        result.failure_reason = "Not a template aggregate";
        return result;
    }

    std::vector<std::shared_ptr<type>> arg_types;
    for (auto& a : args) {
        arg_types.push_back(a ? a->get_type() : nullptr);
    }

    auto deduction_res = deduce_aggregate_ctad_candidates(*tpl_agg, arg_types);
    if (!deduction_res.success || deduction_res.candidates.empty()) {
        result.success = false;
        result.failure_reason = deduction_res.failure_reason;
        return result;
    }

    struct CandidateMatch {
        std::shared_ptr<aggregate> concrete_agg;
        std::shared_ptr<struct_type> concrete_st;
        std::shared_ptr<constructor> selected_ctor;
        std::vector<template_argument> deduced_args;
        std::vector<std::shared_ptr<expression>> adapted_args;
        cast_weight score = CAST_NONE;
        size_t defaults_used = 0;
        bool is_copy_guide = false;
    };

    std::vector<CandidateMatch> viable_matches;

    for (const auto& cand : deduction_res.candidates) {
        auto concrete_st_raw = instantiate_template_aggregate(
            tpl_agg, cand.deduced_args, context_elem);
        auto concrete_st = std::dynamic_pointer_cast<struct_type>(concrete_st_raw);
        if (!concrete_st) continue;
        auto concrete_agg = concrete_st->get_struct();
        if (!concrete_agg) continue;

        if (cand.is_copy_guide) {
            CandidateMatch m;
            m.concrete_agg = concrete_agg;
            m.concrete_st = concrete_st;
            m.selected_ctor = nullptr;
            m.deduced_args = cand.deduced_args;
            m.adapted_args = args;
            m.score = CAST_NONE;
            m.defaults_used = 0;
            m.is_copy_guide = true;
            viable_matches.push_back(std::move(m));
            continue;
        }

        if (cand.is_default_guide) {
            auto constructors = concrete_agg->constructors();
            std::shared_ptr<constructor> default_ctor = nullptr;
            for (auto& c : constructors) {
                if (c->parameters().empty() || std::all_of(c->parameters().begin(), c->parameters().end(),
                    [](const auto& p) { return p->has_default_expr(); })) {
                    default_ctor = c;
                    break;
                }
            }
            ...
            m.concrete_agg = concrete_agg;
            m.concrete_st = concrete_st;
            m.selected_ctor = default_ctor;
            m.deduced_args = cand.deduced_args;
            m.adapted_args = {};
            m.score = CAST_NONE;
            m.defaults_used = 0;
            m.is_copy_guide = false;
            viable_matches.push_back(std::move(m));
            continue;
        }

        // Explicit constructor candidate
        auto constructors = concrete_agg->constructors();
        if (constructors.empty()) continue;

        auto [best_ctor, adapted] = get_best_matching_constructor(constructors, args);
        if (best_ctor) {
            cast_weight total_score = CAST_NONE;
            for (size_t i = 0; i < args.size() && i < best_ctor->parameters().size(); ++i) {
                auto w = compute_cast_weight(args[i], best_ctor->parameters()[i]->get_type());
                if (w > total_score) total_score = w;
            }
            size_t defaults = 0;
            if (best_ctor->parameters().size() > args.size()) {
                defaults = best_ctor->parameters().size() - args.size();
            }
            CandidateMatch m;
            m.concrete_agg = concrete_agg;
            m.concrete_st = concrete_st;
            m.selected_ctor = best_ctor;
            m.deduced_args = cand.deduced_args;
            m.adapted_args = std::move(adapted);
            m.score = total_score;
            m.defaults_used = defaults;
            m.is_copy_guide = false;
            viable_matches.push_back(std::move(m));
        }
    }

    if (viable_matches.empty()) {
        result.success = false;
        result.failure_reason = "No viable constructor found for deduced template arguments";
        return result;
    }

    // Rank viable candidates
    // Prefer copy guide if exact match
    for (const auto& vm : viable_matches) {
        if (vm.is_copy_guide) {
            result.success = true;
            result.concrete_aggregate = vm.concrete_agg;
            result.concrete_struct_type = vm.concrete_st;
            result.selected_constructor = vm.selected_ctor;
            result.deduced_args = vm.deduced_args;
            result.adapted_args = vm.adapted_args;
            result.is_copy_guide = true;
            return result;
        }
    }

    auto best_it = viable_matches.begin();
    for (auto it = viable_matches.begin() + 1; it != viable_matches.end(); ++it) {
        if (it->score < best_it->score) {
            best_it = it;
        } else if (it->score == best_it->score) {
            if (it->defaults_used < best_it->defaults_used) {
                best_it = it;
            }
        }
    }

    for (auto it = viable_matches.begin(); it != viable_matches.end(); ++it) {
        if (it != best_it && it->score == best_it->score && it->defaults_used == best_it->defaults_used) {
            if (it->concrete_agg != best_it->concrete_agg) {
                result.success = false;
                result.failure_reason = "Ambiguous class template argument deduction for '" +
                    tpl_agg->get_short_name() + "'";
                return result;
            }
        }
    }
    ...
    result.success = true;
    result.concrete_aggregate = best_it->concrete_agg;
    result.concrete_struct_type = best_it->concrete_st;
    result.selected_constructor = best_it->selected_ctor;
    result.deduced_args = best_it->deduced_args;
    result.adapted_args = best_it->adapted_args;
    result.is_copy_guide = best_it->is_copy_guide;
    return result;
}
    ...
    const element& context_elem)
{
    ...
    // 1. Look up the template aggregate by base name
    std::shared_ptr<aggregate> tpl_agg = find_template_aggregate(base_name, context_elem);
    ...
    return instantiate_template_aggregate(tpl_agg, model_args, context_elem);
}
...
{
    if (auto pinned = type::make_pinned_wrapper(model, sub)) {
        return pinned;
    }
...
type_reference_resolver::compute_cast_weight(const std::shared_ptr<expression>& expr, const std::shared_ptr<k::model::type>& tgt) {
    ...
    if (type::is_pointer(type_src)) {
        ...
        if (type::is_reference(tgt_nc)) {
            ...
            if (src_sub_nc == tgt_sub_nc || type::are_equal(src_sub_nc, tgt_sub_nc) || types_match_array_const_compatible(src_sub_nc, tgt_sub_nc))
                return CAST_WIDENING;
        }
        return CAST_IMPOSSIBLE;
    }
    ...
    if (type::is_reference(effective_src)) {
        ...
        if (sub == tgt_nc || type::are_equal(sub, tgt_nc)) {
            return CAST_REF_CONV;
        }
    ...
    if (type::is_reference(tgt_nc) && type::is_reference(effective_src)) {
        auto src_ref_sub = std::dynamic_pointer_cast<reference_type>(effective_src)->get_referenced_type();
        auto tgt_ref_sub = std::dynamic_pointer_cast<reference_type>(tgt_nc)->get_referenced_type();
        auto src_st_type = std::dynamic_pointer_cast<struct_type>(type::remove_const(src_ref_sub));
        auto tgt_st_type = std::dynamic_pointer_cast<struct_type>(type::remove_const(tgt_ref_sub));
        if (src_st_type && tgt_st_type && src_st_type != tgt_st_type) {
            if (type::is_const(src_ref_sub) && !type::is_const(tgt_ref_sub)) {
                return CAST_IMPOSSIBLE;
            }
            auto src_st = src_st_type->get_struct();
            auto tgt_st = tgt_st_type->get_struct();
...
type_reference_resolver::get_best_matching_constructor(const std::vector<std::shared_ptr<constructor>>& constructors, const std::vector<std::shared_ptr<expression>>& args) {
    ...
    for (auto& ctor : arity_matched) {
        ...
        for (size_t i = 0; i < arg_count; ++i) {
            ...
            if (param_type && !type::is_resolved(param_type)) {
                visit_parameter(*params[i]);
