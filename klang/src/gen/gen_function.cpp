...
void declaration_generator::visit_function(function &function) {
    ...
    // deleted, redirected, defaulted, or intrinsic, it is an error. This catches functions that
    ...
    if (function.get_ast_function_decl() && !function.get_ast_function_decl()->content
        ...
        && !get_intrinsic_name(function).has_value()) {
        lex::opt_any_lexeme fn_lexeme;
...
void implementation_generator::visit_function(function &function) {
    ...
    // Exception: intrinsic functions have their body generated by the compiler.
    if (!function._block && !function.is_compiler_generated()) {
        if (!get_intrinsic_name(function).has_value()) {
            return;
        }
    }
    ...
    // ── Intrinsic dispatch ────────────────────────────────────────────────────
    // Functions annotated with @Intrinsic("...") bypass normal body generation.
    if (auto intrinsic = get_intrinsic_name(function)) {
        if (*intrinsic == "UniSlot::constructor" || *intrinsic == "UniSlot::destructor") {
            // Empty body: just emit ret void — no member ctor/dtor calls.
            // (zero-init was already done by emit_constructor_pre_block for ctors)
            _builder->CreateRetVoid();
            llvm::verifyFunction(*func);
            return;
        } else if (*intrinsic == "UniSlot::construct") {
            emit_intrinsic_unislot_construct(function, func);
            return;
...
void implementation_generator::emit_destructor_cleanup(function& function) {
    ...
    // Intrinsic destructors do not auto-destruct members.
