...
class ast_dump_visitor : public k::parse::ast_visitor {
    ...
    public:
        ...
        void visit_const_type_specifier(ast::const_type_specifier &ct) override {
            _stm << "const ";
            ct.subtype->visit(*this);
        }

        void visit_callable_type_specifier(ast::callable_type_specifier &frt) override {
            if (frt.owner.has_value()) {
                frt.owner->visit(*this);
                _stm << "::";
            }
            if (frt.addresser.has_value()) _stm << frt.addresser->content;
            _stm << "(";
        ...
        void visit_using_decl(ast::using_decl& decl) override {
            prefix() << "using ";
        ...
        void visit_alias_decl(ast::alias_decl& decl) override {
            ...
            if (decl.is_template()) {
                ...
                for (auto& tp : decl.template_params) {
                    if (!first) _stm << ", ";
                    first = false;
                    _stm << std::string{tp->name.content};
                }
                _stm << "> ";
            }
            _stm << (decl.is_strong ? "typedef " : "alias ")
        ...
        virtual void visit_break_statement(ast::break_statement& stmt) override {
        ...
        virtual void visit_foreach_statement(ast::foreach_statement& stmt) override {
            prefix() << "for ( ";
            stmt.decl_expr->visit(*this);
            _stm << " ) " << std::endl;
            auto pf = prefix_inc();
