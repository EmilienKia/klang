...
cbor_item_t* encode_union(const kdi_union& u) {
    ...
    if (!u.polymorphic_base_fq_name.empty())
        map_push(m, "polymorphic_base", cbor_str(u.polymorphic_base_fq_name));
    if (!u.llvm_def.empty())
...
kdi_union decode_union(cbor_item_t* item, const std::string& path) {
    ...
    u.polymorphic_base_fq_name  = opt_string(item, "polymorphic_base");
